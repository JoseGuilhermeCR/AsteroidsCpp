      ...
      m_currentState(GameState::IN_MENU)
{
    ...
    // Highscore Texts
    m_uiManager.addUiText("Highscore: " + to_string(m_highscoreManager.getSavedScore()), "highscore_highscore", 64,
    ...
    m_uiManager.setEvent("Clear Highscore", [this] () { m_highscoreManager.setNewHighscore(0);
                                                        m_uiManager.setText("highscore_highscore", "Highscore: " + to_string(m_player.getScore()));
                                                      });
    ...
    if (m_currentState == GameState::IN_GAME) {
        ...
        if (!m_paused) {
            ...
            if (m_player.getLifeNumber() == 0) {
                m_uiManager.setText("gameover_scored", "Your score: " + to_string(m_player.getScore()));

                if (m_player.getScore() > m_highscoreManager.getSavedScore()) {
                    m_uiManager.setText("gameover_newhighscore", "New Highscore: " + to_string(m_player.getScore()) + "!");
...
template <typename T> std::string Game::to_string(const T &n)
{
