...
void Player::update(const float deltaTime) {
    ...
    //std::cout << "Current velocity: (" << m_velX << ", " << m_velY << ")" << std::endl;
    ...
    if (m_vulnerable) {
        ...
        for (Asteroid &asteroid : asteroids) {
            for (unsigned int i = 0; i < m_shape.getPointCount(); ++i) {
                ...
                float dist = sqrt(pow(asteroid.getX() - (m_shape.getPosition().x + point.x), 2) + pow(asteroid.getY() - (m_shape.getPosition().y + point.y), 2));
                if (dist < asteroid.getR() + asteroid.getAverageOffset()) {
                    x = Utils::WIDTH / 2;
                    y = Utils::HEIGHT / 2;
    ...
    if (m_canShoot) {
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
            ...
            m_shootTimer = 0;
            m_canShoot = false;
	    //m_soundManager.playSound("playerShoot");
        }
    }
    ...
    for (unsigned int i = 0; i < m_lasers.size(); ++i) {
        ...
        for (unsigned int j = 0; j < asteroids.size(); ++j) {
            ...
            if (m_lasers.at(i).hits(asteroids.at(j))) {
                ...
                m_uiManager.setText("ingame_scorecount", "Score: " + to_string(m_score));
		//m_soundManager.playSound("explosionLaserAsteroid");
            }
        }
    }
}
...
void Player::setLifeNumber(unsigned int lifeNumber) {
    m_uiManager.setText("ingame_lifecount", "Lifes: " + to_string(lifeNumber));
    m_lives = lifeNumber;
}
...
template <typename T> std::string Player::to_string(const T &n) {
