...
BossManager::BossManager(Player& player, ParticlesManager& particlesManager)
    	: 
	m_player(player),
      	m_boss(particlesManager, player),
      	...
      	m_time(m_timer)
{
    	...
    	sf::Vector2f position = m_possibleSpawnLocations.at(Utils::randomRange(0, 3));
    	sf::Vector2f velocity = m_possibleVelocities.at(Utils::randomRange(0, 3));
    	...
    	if (m_bossAlive)
	{
        	...
        	for (unsigned int i = 0; i < playerLasers.size(); ++i)
		{
            		...
            		for (unsigned int j = 0; j < m_bossShape.getPointCount(); ++j)
			{
                		...
                		float d = Utils::dist(m_boss.getPosition().x + point.x, pos.x, m_boss.getPosition().y + point.y, pos.y);
                		if (d < 20)
				{
