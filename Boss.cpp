...
Boss::Boss(ParticlesManager& particleManager, Player& player)
    	: 
	m_shape(),
      	m_line1(),
      	m_line2(),
...
void Boss::update(float deltaTime)
{
    	...
    	if (m_player.isPlayerVulnerable())
	{
        	// Shoot
    	...
    	const sf::ConvexShape &playerShape = m_player.getShapeFromPlayer();
    	// Update the lasers
    	for (unsigned int i = 0; i < m_lasers.size(); ++i)
	{
        	...
        	for (unsigned int j = 0; j < playerShape.getPointCount(); ++j)
		{
            		...
            		float d = Utils::dist(m_lasers.at(i).getX(), m_player.getX() + point.x, m_lasers.at(i).getY(), m_player.getY() + point.y);
...
const sf::ConvexShape& Boss::getShape()
{
	return m_shape;
}
